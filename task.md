...
#### 任务 2：types/common.go —— Logger 接口 + 错误类型
...
#### 任务 3：utils.go —— GenerateReqId / GenerateRandomString
...
#### 任务 6：types/api.go（上）—— WsCmd / WsFrame / WsFrameHeaders
...
#### 任务 9：types/index.go —— 包内 re-export
...
#### 任务 10：aibot/index.go —— 跨包 re-export（Go 1.24 泛型别名）🏁
...
#### 任务 14：ws.go（末）—— 串行回复队列 + ack
- 文件（修改）：`aibot/ws.go` —— `SendReply(frame WsFrameHeaders, body, cmd)`（同一 reqId 串行队列）、`replyProcessor`（发一条→等 ack/5s 超时→下一条）、`pendingAcks`+seq 防竞态、`HasPendingAck`
- 验证：mock 测试——回复收到 ack 解析成功；不回则超时报错；同 reqId 串行有序

### E. 客户端门面（client.go / message-handler.go）

#### 任务 15：client.go（上）—— WsClient + 连接 + 回调
- 文件（修改）：`aibot/client.go` —— `WsClient` 结构（持有 `WsConnectionManager`/回调字段 `OnMessage`/`OnText`/…/`OnConnected`/`OnAuthenticated`/`OnDisconnected`/`OnReconnecting`/`OnError`）+ `NewWsClient(opts WsClientOptions)`（兜底默认值）+ `Connect(ctx)`/`Disconnect()`/`IsConnected()` + 回调桥接
- 验证：mock 测试——连接+认证成功触发 `OnAuthenticated`

#### 任务 16：message-handler.go —— MessageHandler 分发
- 文件（修改）：`aibot/message-handler.go` —— `MessageHandler.handleFrame`（镜像 Node：按 cmd+msgtype 二次解码触发回调；先文本/事件，后续任务补全其余类型）
- 验证：mock 测试——服务端推文本/事件帧，`OnText`/`OnEvent` 触发且 body 正确

#### 任务 17：client.go（中）—— Reply / ReplyStream / ReplyStreamNonBlocking
- 文件（修改）：`aibot/client.go` —— `Reply(frame WsFrameHeaders, body, cmd?)`/`ReplyStream`/`ReplyStreamNonBlocking`（非阻塞：上一条未 ack 且非最终帧返回 `ErrReplySkipped`）
- 验证：mock 测试——流式中间帧+最终帧；非阻塞跳过路径
