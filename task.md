...
#### 任务 18：最小闭环示例与文档 🏁（首个「最小可验证功能」里程碑） ✅ 已完成
- 文件（新增）：`examples/basic/main.go` —— 连接+认证+生命周期回调+收文本（OnMessage/OnText）+流式回复（中间帧+最终帧）+SIGINT/SIGTERM 优雅退出（凭证走环境变量）
- 文件（新增）：`README.md` —— 简介/状态/安装/import/快速开始（最小闭环）/配置表/核心 API 表/构建测试命令
- 验证：`go build ./examples/basic` 通过；`go build ./...` + `go vet ./...` + `go test ./...` 全通过；README API/配置与代码一致
...
#### 任务 19：message-handler.go —— 其余消息/事件回调 ✅ 已完成
- 文件（修改）：`aibot/message-handler.go` —— `handleMessageCallback` 补全 image/mixed/voice/file/video 分发（泛型 `dispatchTyped[T]` 去重）；`handleEventCallback` 按 eventtype 分发 OnEnterChat/OnTemplateCardEvent/OnFeedbackEvent/OnDisconnectedEvent（DecodeEvent 填充 Body.Event）；`aibot/ws.go` —— `handleFrame` EventCallback 分支检测 `disconnected_event`，`handleServerDisconnect`（stopHeartbeat→clearPendingMessages→isManualClose→closed→OnServerDisconnect→关 socket，对应 Node）+ `isDisconnectedEvent` 探针。回调字段已于任务 15 全部声明，`client.go` 无需改动
- 验证：mock 测试——各 msgtype/eventtype 触发对应回调且 body 正确；`disconnected_event` 触发 OnDisconnectedEvent+OnDisconnected 且不重连（isManualClose）

#### 任务 20：crypto.go —— 文件附件解密 DecryptFile ✅ 已完成
- 文件（修改）：`aibot/crypto.go` —— `DecryptFile(encrypted, aesKey)`（Base64 解码 aesKey、AES-256-CBC、IV=key[:16]、关闭自动 padding 后手动去填充）、`Pkcs7Pad`/`Pkcs7Unpad`/`Pkcs7BlockSize=32` 常量（按缩写命名约定 PKCS→Pkcs）
- 文件（新增）：`aibot/crypto_test.go`
- 验证：单测往返（变长/边界）+ fuzz（27k+ 用例通过）+ 错误 key 不还原原文 + 空输入/非法 base64/非法 key 长度/非对齐密文均失败 + Pkcs7 单测

#### 任务 21：api.go —— WeComApiClient HTTP 下载 ✅ 已完成
- 文件（修改）：`aibot/api.go` —— `WeComApiClient.DownloadFileRaw(url)`（解析 `Content-Disposition`，优先 RFC5987 `filename*=UTF-8''`）
- 文件（新增）：`aibot/api_test.go`
- 验证：单测 `Content-Disposition` 多格式解析（RFC5987 UTF-8/ASCII/小写、quoted/unquoted/大小写不敏感、优先级、空格截断、空 header）+ `+` 保留 + 非法转义报错 + httptest 集成（RFC5987/quoted/无 CD/404/超时/默认超时兜底）

#### 任务 22：client.go —— DownloadFile ✅ 已完成
- 文件（修改）：`aibot/client.go` —— `DownloadFile(url, aesKey)`（下载 + 调 `DecryptFile`，无 aesKey 返回原文并告警）；接入 `apiClient` 字段 + `Api()` getter（对应 Node `get api()`）
- 文件（修改）：`aibot/client_test.go` —— 追加 DownloadFile 测试
- 验证：mock http + 加密往返解密（文件名从 Content-Disposition 解析）；无 aesKey 返回原文；错误 aesKey 报错；404 下载失败报错；Api() getter 非空

#### 任务 23：wecom-crypto.go —— WecomCrypto（Webhook 加解密） ✅ 已完成
- 文件（修改）：`aibot/wecom-crypto.go` —— `DecodeEncodingAesKey`（43 位补 `=` → base64 → 32 字节）、`WecomCrypto`（`NewWecomCrypto`/`ComputeSignature`/`VerifySignature`/`Decrypt`/`Encrypt`，明文结构 `[16随机][4大端len][msg][receiveId]`，PKCS#7(32) 复用 crypto.go）
- 文件（新增）：`aibot/wecom-crypto_test.go`
- 验证：DecodeEncodingAesKey 多场景（合法/带填充/空/非法长度）+ 构造错误 + 签名格式(40hex)/校验 + 加解密往返（含变长/中文/空）+ receiveId 匹配/不匹配/空 + Decrypt 非法输入 + fuzz（42k+ 用例通过）

#### 任务 24：types/api.go（下）—— 模板卡片 + 回复体类型 ✅ 已完成
- 文件（修改）：`aibot/types/api.go` —— 追加 `TemplateCardType` + `TemplateCard` 及全部子结构（Source/ActionMenu/MainTitle/EmphasisContent/QuoteArea/HorizontalContent/JumpAction/Action/VerticalContent/Image/ImageTextArea/SubmitButton/SelectionItem/Button/Checkbox）+ 回复体（`ReplyMsgItem`/`ReplyFeedback`/`StreamReply`/`StreamReplyBody`/`WelcomeTextReplyBody`/`WelcomeTemplateCardReplyBody`/`TemplateCardReplyBody`/`StreamWithTemplateCardReplyBody`/`UpdateTemplateCardBody`）；可选子结构用指针保证 omitempty 生效
- 文件（修改）：`aibot/types/api_test.go` —— 追加卡片/回复体单测；`aibot/types/index.go` —— 补充新类型 manifest
- 验证：TemplateCardType 常量 + TemplateCard 最小→可选省略（仅 card_type）+ `TemplateCardAction.Type=0` 必填保留 + 填充往返（含匿名结构切片）+ ReplyMsgItem 精确 JSON + StreamReply omitempty + 各回复体 msgtype + StreamWithTemplateCard 可选卡片 + UpdateTemplateCardBody userids 省略/出现

#### 任务 25：client.go —— 卡片与欢迎语回复 ✅ 已完成
- 文件（修改）：`aibot/client.go` —— `ReplyWelcome`（cmd=ResponseWelcome）/`ReplyTemplateCard`（feedback 非空合并进卡片，不改调用方原 card）/`ReplyStreamWithCard`（`ReplyStreamWithCardOptions` 结构体承载 4 可选项：msgItem/streamFeedback/templateCard/cardFeedback）/`UpdateTemplateCard`（cmd=ResponseUpdate，userIds 非空附带）；测试追加到 `aibot/client_test.go`
- 文件（修改）：`aibot/types/api.go` —— 修正 `StreamReply.Finish/Content` 去掉 omitempty（Node 总是序列化 id/finish/content，仅 msg_item/feedback 条件序列化；与现有 ReplyStream 一致）；同步 `aibot/types/api_test.go`
- 验证：mock 捕获 cmd+body——ReplyWelcome 文本帧；ReplyTemplateCard feedback 合并/不合并且不污染调用方 card；ReplyStreamWithCard 全字段 + 无卡片场景；UpdateTemplateCard userIds 有/无 + ResponseUpdate cmd

#### 任务 26：client.go —— 主动发送与媒体回复 ✅ 已完成
- 文件（修改）：`aibot/types/api.go` —— 追加 `WeComMediaType`(+4 常量)、`SendMediaContent`/`SendVideoContent`、`SendMediaMsgBody`(file/image/voice/video 四选一)、`SendMarkdownMsgBody`、`SendTemplateCardMsgBody`；`aibot/types/index.go` 补 manifest
- 文件（修改）：`aibot/client.go` —— `SendMessage`（生成新 reqId + chatid 合并进 body + SendMsg cmd）、`SendMediaMessage`、`ReplyMedia`（被动回复，不合并 chatid）；`VideoOptions` + `buildMediaBody`(switch 对应 Node 动态键) + `mergeChatId` 辅助；测试追加到 `aibot/client_test.go`（mock 加 SendMsg ack）
- 验证：mock 捕获 cmd+body——SendMessage markdown/chatid 合并 + SendMsg cmd；SendMediaMessage image(仅 image 键) + video(title/description)；ReplyMedia file(无 chatid + Response cmd + 仅 file 键)

#### 任务 27：client.go —— UploadMedia 分片上传 ✅ 已完成
- 文件（修改）：`aibot/types/api.go` —— 追加上传类型（`UploadMediaOptions`/`UploadMediaInitBody`/`UploadMediaInitResult`/`UploadMediaChunkBody`/`UploadMediaFinishBody`/`UploadMediaFinishResult`，chunk_index 从 0）；`aibot/types/index.go` 补 manifest
- 文件（修改）：`aibot/client.go` —— `UploadMedia(fileBuffer, opts)`：init→chunk×N→finish；512KB/片、md5、>100 分片拒绝；动态并发（≤4/3/2 worker pool + channel 派发）、单片重试 2 次（500ms×n 退避）；type/created_at 缺省回退
- 文件（修改）：`aibot/client_test.go` —— 上传 mock（init/chunk/finish 带 body，可注入分片失败）
- 验证：单片往返返回 media_id；多分片(3)并发往返；重试路径(chunk 0 首次失败→重试成功，0.5s)；超大文件(101 分片)拒绝且不发起上传

#### 任务 28：index.go / types/index.go 完整 re-export 校对 + 完整文档 🏁 ✅ 已完成
- 文件（修改）：`aibot/index.go` —— 补全任务 24/26/27 新增类型的跨包别名（卡片 15 子结构 + 7 回复体 + WeComMediaType/4 常量 + 媒体/发送体 + 上传 6 类型 + TemplateCardType var），使 `aibot.X` 全可用；`aibot/index_test.go` 追加 3 个 re-export 冒烟测试
- 文件（修改）：`README.md` —— 重写为完整文档（特性/安装/快速开始/配置/核心 API 全表/消息类型/事件类型/WsCmd 协议常量/加解密/项目结构）
- 文件（修改）：`CLAUDE.md` —— 追加「架构总览」（WsClient 门面聚合 ws/handler/api + 收发/上传/加解密数据流）
- 验证：`gofmt -l` 无差异 + `go build ./...` + `go vet ./...` + `go test ./...` 全通过（11 个 re-export 测试 + 全量回归）；文档 API 与代码一致

---

> 🏁 全部 28 个任务完成：Node SDK 1:1 镜像 Go 实现闭环（连接/认证/心跳/重连/回复队列/消息事件分发/流式+卡片+欢迎语回复/主动发送/媒体/分片上传/文件下载解密/Webhook 加解密）。
